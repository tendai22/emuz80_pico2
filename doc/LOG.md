...
これで Z80 を挿すと動作するようになった。

* all NOP, 20FE(JMP $-2: 相対ジャンプ命令を無限ループ)で動作している様子。Write Cycle の動作は不明。
* クロックを 3MHz(およそ)より速くすると、M1 サイクルで 1wait が入る。WAIT 信号は期待通りに出力されている様子。
* メモリアクセス時には WAIT なしにすることで3MHz よりもクロックを上げることができそう。
* コンパイルオプションは `target_compile_options(emuz80_pico2 PUBLIC -Ofast)` で指定できる。
  + `-O3`, `-O2`, `-Ofast` を試したが、MREQ から WAIT の上がりエッジ(データバスへの命令出力完了)までの時間は変わらなかった。
* `set pins, 1`(WAITをHighにする)を、データバス出力完了の直後まで繰り上げたが、ノーウェイトを維持しながらのクロック高速化にはあまり貢献できていない。PIO内部の処理は時間が短い(1命令8ns程度で、数命令短縮)だからだろう。
* UARTアクセス以外(通常のメモリアクセス)では WAIT を Low にしないことでさらに半クロック分は余裕がでてくるのではないか。
  + アドレス A13,14,15 が all 1 でない場合(EMUZ80 では E00XにUARTを置いている)に WAIT 信号を出さないようにする。
  + A13,14,15,MREQ を wait で待ち、INT3 を立てる。
  + INT2で従来の WAIT を起こす。
  + wait 0 irq 2/wait 1 irq 3がWAITなし版の最初、wait 1 irq 2/wait 0 irq 3 が WAITあり版の最初とする。
  + PIO3つ目を使うことになるだろう。
...
* wait_mreq: mreq/rfsh 監視を wait 2命令にまとめる(2命令短縮できる)
* uart統合: 以前「stdio_uart の uart status 処理」として考察済み。このアイディアで組み込む。
* PIO ピンの初期化を C-SDK 側に移す。(PIOアセンブリコード短縮のため)
* ROMデータ外部からロード: とりあえずショート形式をサポート。外に出すには HEX 読み込みも組み込む。シリアルロードと結果ダンプまで。モニタまで広げない。
...
時間のかかる処理を core0 に回して、core1 では変数参照とレジスタ値作成だけを行う。

core0 と core1 の間では変数の参照だけで情報伝達する。core1 で時間のかかる処理は行わない。

変数参照だけだと、WAIT == L になるのは 200ns - 250ns 程度となった。IO R/W 命令だと実質ノーウェイトで実行できる。
...
* 送信データ書き込み可能を表す。
* CDCスタック側で書き込みバッファに空きができると core0 で 2 を立てる。
* core1側では、Z80がステータスレジスタを読み込むときに OR で束ねてステータスレジスタとして返す。
* Z80側では、このビット(bit1)を見て TXデータレジスタに出力データを書き込む。
* core1側で、TXデータレジスタに書き込まれると、tx_data にその値を書き込み、txbuf_full を立てる。
* core0側で、txbuf_full が立っていれば、CDCスタックに送信データを書き出し、書き込み完了まで待つ(flushの呼び出しによる)。
* 書き込み完了後に tx_rdy、txbuf_full をクリアする。

### rx_rdy

* 受信データ到着(取り出し可能)を表す。
* core1側では、Z80がステータスレジスタを読み込むときに OR で束ねてステータスレジスタとして返す。
* core0側では、rx_rdy == 0 かつCDCスタック側に受信データが到着していれば、
* 受信データを取り出し rx_data にセットするとともに、rx_rdy に1をセットする。
* Z80側では、ステータスレジスタのbit0が立っていれば、データレジスタを読み込む
* データレジスタの読み込みがあれば、rx_data を返すとともに、rx_rdy をクリアする。

### コア間通信の正当性

読込みについて、以下のルールが成り立っていれば、タスク間の齟齬は発生しない。

* rx_rdy に 1 を書き込むのは rx_rdy が 0 && CDCデータが到着している時に限る。
* rx_rdy に 1 を書き込む前に、rx_data には読込みデータが設定しておく。
* Z80が rx_data を読みだした後で rx_rdy に 0 を設定する。

書き込みについて、

* tx_rdy に 1 を書き込むのは、tx_rdy が 0 && CDC出力が可能なときに限る。
* tx_data にデータを設定した直後に txbuf_full に 1 を設定する。 
* txbuf_full が 1 かつ tx_rdy が非ゼロのとき、core0 は tx_data の値をCDCスタックに出力する。
* tx_dataの値を出力すると、txbuf_full, tx_rdy をともにクリアする。

### volatile 宣言

volatile宣言を入れておかないと、status = (rx_rdy | tx_rdy) のビットが立たない。単一コア内部で最適化が効いていると、他コアで tx_rdy に 2 を設定しても status レジスタの値に反映されず 0 のままである。
