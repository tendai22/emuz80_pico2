...
## WR を GPIO26 に移して 1kΩ抵抗を挟む

IO Write サイクルで IORQ == High, WR == Low の状態が生じるのがよくない。

上記対処では、再度 `port = gpio_get_all()` することで安定状態の入力を得ていたが、

IORQ が Low になった時点で WR が Low になっていてほしい。両方直結だと WR が微妙に先にアサートしているようだ。ということで、WR と GPIOピンの間に 1kΩ 抵抗を挟んで WR を遅らせる。

GPIO26以後で入力扱いする場合、1kΩ抵抗を挟んでGPIO端子を保護するので、WRを GPIO26に移す。

これで再度 `port = gpio_get_all()` しなくても動作する。確実に動作させたい場合は、Read ループを抜けた時点で再度 `port = gpio_get_all()` して、メモリライトサイクルかどうかを判断するようにするとよいが、その分 WR サイクルが遅くなる。

ひょっとすると、ループ先頭を(メモリリード判定でなく)メモリライトサイクルとしたほうがよいかもしれない。
...
GPIO24,25 は FT属性なので、Z80と直結で構わない。GPIO26はZ80出力・RP2040側入力なので、1kΩ抵抗を挟んで気休めでも端子を保護する。GPIO27,28,29はGPIO側出力、Z80側入力なので、直結で構わない。
